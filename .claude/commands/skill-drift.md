...
This repo (`tapkit-plugins-claude`) mirrors skills from `jootsing-research/skills`. The two are intended to stay in lockstep for **skills** — only the plugin shell differs (`.claude-plugin/` here vs the upstream repo's packaging). Your job is to surface any skill drift between the two so the user can decide what to mirror over.
...
- `.claude-plugin/plugin.json` or any plugin shell files
- `assets/` (logos, images)
...
3. **Diff each shared skill's `SKILL.md`.** For every skill that exists in both, fetch the upstream contents and diff against the local file. Use parallel WebFetch calls for the raw URLs to keep this fast:
   ...
   - **Adapt** — upstream-specific reference that needs translation for Claude (e.g. mentions of a different plugin shell, Codex/other-host tool names, non-Claude frontmatter conventions).
   - **Ignore** — already correct difference between the two mirrors (the Claude side intentionally diverges here).
